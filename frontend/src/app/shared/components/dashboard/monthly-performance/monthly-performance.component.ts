...
export class MonthlyPerformanceComponent {
  @Input() title = 'Objectif mensuel';
  @Input() subtitle = 'Suivi des operations du mois';
  @Input() progress = 75.55;
  @Input() deltaPercent = 10;
  @Input() summaryText = "Aucune activite enregistree pour le moment.";
  @Input() targetLabel = 'Objectif';
  @Input() revenueLabel = 'Traite';
  @Input() todayLabel = "Aujourd'hui";
  ...
  public labels: string[] = ['Progression'];
  ...
  private formatCompactValue(value: number): string {
    ...
    const rounded = `${Math.round(absValue).toLocaleString('fr-FR')}`;
    if (this.valueFormat === 'currency') {
